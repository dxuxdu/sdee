  ...
  return (
    <>
      ...
      {activeVideo && (
        ...
        >
          <div
            className="w-full max-w-4xl bg-[#101010] rounded-xl border border-[#333] overflow-hidden shadow-2xl"
          ...
          >
            ...
            <div className="p-4 flex justify-between items-center bg-[#141414]">
              <h3 className="text-white font-semibold text-sm line-clamp-1">{activeVideo.title}</h3>
              <button
                onClick={() => setActiveVideo(null)}
          ...
          {marqueeVideos.map((video, index) => (
            <div key={`${video.id}-${index}`} className="w-[280px] md:w-[320px] flex-shrink-0">
              <button
                ...
                className="block w-full relative aspect-video rounded-xl overflow-hidden border border-white/10 bg-[#1a1a1a] transition-transform duration-300 hover:scale-105 hover:border-red-500/50 group/card"
              >
                  {/* Thumbnail */}
                  <img 
                      src={video.thumbnail} 
